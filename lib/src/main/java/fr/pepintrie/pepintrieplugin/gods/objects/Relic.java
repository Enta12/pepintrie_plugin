...
public abstract class Relic {
	...
	private ItemMeta getItemMeta(ItemStack relicItem){
		ItemMeta customM = relicItem.getItemMeta();
		customM.addEnchant(Enchantment.FROST_WALKER , 255, true);
		return customM;
	}
	...
	public static ItemStack getNetherRelic(int utilisationLeft, String godName, String altar,String object,  int powerNeed) {
		...
		ItemMeta customM = getItemMeta(relicItem);
	...
	public static ItemStack getSeaRelic(int utilisationLeft, String godName, String object,  int powerNeed) {
		...
		boolean haveFindAnInterestingBiome = false;
		int numberOfTry = 0;
		String biomeString;
		Biome biome = "un biome peu interessant ...";

		while(!haveFindAnInterestingBiome && numberOfTry<100){
			x1 = random.nextInt(5);
			x2 = random.nextInt(99);
			x3 = random.nextInt(99);
			
			z1 = random.nextInt(5);
			z2 = random.nextInt(99);
			z3 = random.nextInt(99);
			biome = Bukkit.getWorld("world").getBiome(x1*10000+x2*100+x3, 60, z1*10000+z2*100+z3);
			...
			if(biome == Biome.BADLANDS || biome == Biome.WOODED_BADLANDS_PLATEAU|| biome == Biome.BADLANDS_PLATEAU || biome == Biome.ERODED_BADLANDS || biome == Biome.MODIFIED_BADLANDS_PLATEAU || biome == Biome.MODIFIED_WOODED_BADLANDS_PLATEAU) {
				...
				biomeString ="une plaine de fleurs";
			}
